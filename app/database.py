...
def _migrate_mxh_schema(conn):
    ...
    try:
        # Check if mxh_cards table exists (new schema)
        cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='mxh_cards'")
        if cursor.fetchone():
            print("SUCCESS: Database already uses new schema. No migration needed.")
            return

        # Check if old mxh_accounts table has old structure
    ...
    try:
        ...
        print("   - Step 2/5: Created new mxh_cards and mxh_accounts tables.")
        ...
        for old_account in old_accounts:
            ...
            cursor.execute(
                "INSERT INTO mxh_cards (id, card_name, group_id, platform, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?)",
                (old_dict['id'], old_dict['card_name'], old_dict['group_id'], old_dict['platform'], old_dict['created_at'], old_dict.get('updated_at'))
            )
            card_id = old_dict['id']
            
            cursor.execute(
                """INSERT INTO mxh_accounts (card_id, is_primary, account_name, username, phone, url, login_username, login_password, created_at, updated_at, wechat_created_day, wechat_created_month, wechat_created_year, wechat_status, muted_until, status, die_date, wechat_scan_count, wechat_last_scan_date, rescue_count, rescue_success_count, notice)
            ...
            if old_dict.get('secondary_username') and old_dict.get('secondary_username').strip() not in ('', '.'):
                cursor.execute(
                    """INSERT INTO mxh_accounts (card_id, is_primary, account_name, username, phone, url, created_at, updated_at, wechat_created_day, wechat_created_month, wechat_created_year, status, muted_until, die_date, wechat_scan_count, wechat_last_scan_date, rescue_count, rescue_success_count)
                       VALUES (?, 0, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
        ...
        try:
            ...
            cursor.execute("DROP TABLE IF EXISTS mxh_cards")
...
def init_database():
    ...
    conn.execute("CREATE TABLE IF NOT EXISTS mxh_cards (id INTEGER PRIMARY KEY AUTOINCREMENT, card_name TEXT NOT NULL, group_id INTEGER NOT NULL, platform TEXT NOT NULL, created_at TEXT NOT NULL, updated_at TEXT, FOREIGN KEY (group_id) REFERENCES mxh_groups(id) ON DELETE CASCADE)")
    conn.execute("CREATE TABLE IF NOT EXISTS mxh_accounts (id INTEGER PRIMARY KEY AUTOINCREMENT, card_id INTEGER NOT NULL, is_primary BOOLEAN DEFAULT 0, account_name TEXT DEFAULT 'Tài khoản phụ', username TEXT, phone TEXT, url TEXT, login_username TEXT, login_password TEXT, created_at TEXT NOT NULL, updated_at TEXT, wechat_created_day INTEGER, wechat_created_month INTEGER, wechat_created_year INTEGER, wechat_status TEXT DEFAULT 'available', muted_until TEXT, status TEXT DEFAULT 'active', die_date TEXT, wechat_scan_count INTEGER DEFAULT 0, wechat_last_scan_date TEXT, rescue_count INTEGER DEFAULT 0, rescue_success_count INTEGER DEFAULT 0, email_reset_date TEXT, notice TEXT, FOREIGN KEY (card_id) REFERENCES mxh_cards(id) ON DELETE CASCADE)")
