    ...
    try:
        if request.method == "GET":
            ...
            if card_ids:
                ...
                accounts = conn.execute(f"SELECT * FROM mxh_accounts WHERE card_id IN ({placeholders}) ORDER BY is_primary DESC, id ASC", card_ids).fetchall()
                for card in result:
                    card['accounts'] = [dict(acc) for acc in accounts if acc['card_id'] == card['id']]
...
def update_delete_account(account_id):
    conn = get_db_connection()
    try:
        if request.method == "PUT":
            data = request.get_json()
            
            # Handle card_name separately (update mxh_cards table)
            if 'card_name' in data:
                card_name = data.pop('card_name')
                conn.execute("UPDATE mxh_cards SET card_name = ?, updated_at = ? WHERE id = (SELECT card_id FROM mxh_accounts WHERE id = ?)", 
                           (card_name, datetime.now().isoformat(), account_id))
            ...
            account = conn.execute("SELECT is_primary FROM mxh_accounts WHERE id = ?", (account_id,)).fetchone()
            if account and account['is_primary']: return jsonify({"error": "Cannot delete primary account"}), 400
...
def add_sub_account(card_id):
    ...
    try:
        now_iso = datetime.now().isoformat()
        cursor = conn.execute("INSERT INTO mxh_accounts (card_id, is_primary, created_at, updated_at) VALUES (?, 0, ?, ?)", (card_id, now_iso, now_iso))
...
def manage_sub_account(sub_account_id):
    ...
    try:
        if request.method == "PUT":
            ...
            conn.execute(f'UPDATE mxh_accounts SET {set_clause} WHERE id = ?', params)
            conn.commit()
            updated = conn.execute("SELECT * FROM mxh_accounts WHERE id = ?", (sub_account_id,)).fetchone()
            return jsonify(dict(updated))
        elif request.method == "DELETE":
            account = conn.execute("SELECT is_primary FROM mxh_accounts WHERE id = ?", (sub_account_id,)).fetchone()
            if account and account['is_primary']: return jsonify({"error": "Cannot delete primary account"}), 400
...
def scan_sub_account(sub_account_id):
    ...
    try:
        ...
        now_iso = datetime.now().isoformat()
        if data.get('reset'):
            conn.execute("UPDATE mxh_accounts SET wechat_scan_count = 0, wechat_last_scan_date = NULL, updated_at = ? WHERE id = ?", (now_iso, sub_account_id))
        else:
            conn.execute("UPDATE mxh_accounts SET wechat_scan_count = wechat_scan_count + 1, wechat_last_scan_date = ?, updated_at = ? WHERE id = ?", (now_iso, now_iso, sub_account_id))
        conn.commit()
...
def toggle_sub_account_status(sub_account_id):
    ...
    try:
        ...
        conn.execute("UPDATE mxh_accounts SET status = CASE WHEN status = 'disabled' THEN 'active' ELSE 'disabled' END, die_date = CASE WHEN status = 'disabled' THEN NULL ELSE ? END, updated_at = ? WHERE id = ?", (now_str, now_iso, sub_account_id))
        conn.commit()
...
def rescue_sub_account(sub_account_id):
    ...
    try:
        ...
        now_iso = datetime.now().isoformat()
        if result == 'success':
            conn.execute("UPDATE mxh_accounts SET status = 'active', die_date = NULL, rescue_success_count = rescue_success_count + 1, updated_at = ? WHERE id = ?", (now_iso, sub_account_id))
            message = "Rescued successfully"
        elif result == 'failed':
            conn.execute("UPDATE mxh_accounts SET rescue_count = rescue_count + 1, updated_at = ? WHERE id = ?", (now_iso, sub_account_id))
...
def mark_sub_account_die(sub_account_id):
    ...
    try:
        now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        now_iso = datetime.now().isoformat()
        conn.execute("UPDATE mxh_accounts SET status = 'disabled', die_date = ?, rescue_count = 0, updated_at = ? WHERE id = ?", (now_str, now_iso, sub_account_id))
        conn.commit()
...
def notice_sub_account(sub_account_id):
    ...
    try:
        if request.method == "PUT":
            ...
            conn.execute("UPDATE mxh_accounts SET notice = ?, updated_at = ? WHERE id = ?", (notice, datetime.now().isoformat(), sub_account_id))
